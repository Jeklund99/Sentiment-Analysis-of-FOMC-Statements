...
for(i in 1:nrow(sentiments)) {
  ...
  month = as.numeric(sentiments[i, "month"]) # Get month that statement was released
  month = month + 1
...
# Create previousMonthCPI variable
...
for(i in 1:nrow(sentiments)) {
  ...
  month = as.numeric(sentiments[i, "month"])
  month = month - 1
  
  # Wrap month value appropriately
  if(month == 0) {
    month = 12
    year = year - 1
  }
  ...
  # Filter CPI data to find the month after the FOMC statement release
  previousMonthCPI <- cpiDataTall %>% filter(Year == year) %>% filter(month == month)
  
  # Add value to the sentiments data frame
...
cpiYearly <- cpiDataTall %>% group_by(Year) %>% dplyr::summarize(meanCPI = mean(CPI, na.rm=TRUE), medianCPI = median(CPI, na.rm=TRUE))
kable(cpiYearly)
...
```{r}
# Resampling method for comparing dependent, overlapping correlations
...
nfciTable <- nfciFiltered %>% group_by(year) %>% dplyr::summarize(meanNFCI = mean(NFCI), medianNFCI = median(NFCI))
kable(nfciTable)
...
anfciTable <- nfciFiltered %>% group_by(year) %>% dplyr::summarize(meanANFCI = mean(ANFCI), medianANFCI = median(ANFCI))
kable(anfciTable)
```
